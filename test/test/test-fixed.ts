...
async function main() {
    ...
    const fixedPaymasterPath = './test/contracts/FixedPaymaster.sol';
    ...
    // send eth to 0xBb6e024b9cFFACB947A71991E386681B1Cd1477D
    ...
    // deploy SingletonFactory to 0xce0042B868300000d44A59004Da54A005ffdcf9f
    try {
    ...
    // deploy FixedPaymaster
    let FixedPaymasterCompile = await Utils.compileContract(fixedPaymasterPath, 'WETHTokenPaymaster');
    let WETHPaymasterContract = new web3.eth.Contract(FixedPaymasterCompile.abi);

    let FixedPaymasterAddress = '';
    try {
        let contract = await WETHPaymasterContract.deploy({
            data: '0x' + FixedPaymasterCompile.bytecode,
            arguments: [
                EntryPointAddress,
                accounts[0],
                WETHAddress,
        ...
        FixedPaymasterAddress = contract.options.address;
    } catch (error) {
        console.log(error);
    }
    console.log('FixedPaymasterAddress: ' + FixedPaymasterAddress);
    ...
    // add stake to FixedPaymaster
    let fixedPaymasterContract = new web3.eth.Contract(FixedPaymasterCompile.abi, FixedPaymasterAddress);
    const addStakeCallData = fixedPaymasterContract.methods.addStake(1).encodeABI();
    const addStakeTx = {
        from: accounts[0],
        to: FixedPaymasterAddress,
    ...
    const depositCallData = fixedPaymasterContract.methods.deposit().encodeABI();
    const depositTx = {
        from: accounts[0],
        to: FixedPaymasterAddress,
    ...
    let walletAddress = await WalletLib.EIP4337.calculateWalletAddress(
        SmartWalletLogicAddress, EntryPointAddress, walletUser.address, WETHAddress, FixedPaymasterAddress, 0, SingletonFactory
    );
    ...
    {
        // estimate gas for activate wallet
        ...
        const _initCode = WalletLib.EIP4337.getWalletCode(SmartWalletLogicAddress, EntryPointAddress, walletUser.address, WETHAddress, FixedPaymasterAddress);
    ...
    const sendErc20Op = await WalletLib.EIP4337.Tokens.ERC20.transferFrom(
        ...
        nonce, EntryPointAddress, FixedPaymasterAddress,
        parseInt(web3.utils.toWei('100', 'gwei')),
        parseInt(web3.utils.toWei('10', 'gwei')),
    ...
    //     SmartWalletLogicAddress, EntryPointAddress, walletUser1.address, WETHAddress, FixedPaymasterAddress, 0, SingletonFactory
    // );
